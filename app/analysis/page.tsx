...
export default function AnalysisDeck() {
  ...
  useLayoutEffect(() => {
    ...
    if (slideParam) {
      ...
      if (!isNaN(slideIndex) && slideIndex >= 0 && slideIndex < analysisSlides.length) {
  ...
  useEffect(() => {
    (window as any).__goToSlide = (index: number) => {
      if (index >= 0 && index < analysisSlides.length) {
  ...
  return (
    <ExportProvider isExport={isExportMode}>
      <div className="h-screen w-screen bg-background overflow-hidden relative flex flex-col">
        {!isExportMode && <RunningAgenda currentSlide={currentSlide} goToSlide={goToSlide} sections={analysisSections} />}
        <main
