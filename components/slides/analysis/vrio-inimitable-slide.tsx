  ...
  return (
    <div className="px-12 pt-8 pb-20 h-full flex flex-col">
      <div className="mb-8">
        ...
        <h2 className="text-5xl font-bold mt-4 tracking-tight text-foreground">Inimitable</h2>
        <p className="text-muted-foreground mt-4 text-2xl">Can competitors easily copy Dexcom's advantages?</p>
      </div>

      <div className="space-y-6 flex-1">
        {/* Inimitable Points */}
        {vrioAnalysis.inimitable.map((point, idx) => (
          <div key={idx} className={`animate-fade-up stagger-${idx + 1} p-6 rounded-lg bg-card border-l-4 border-l-amber-500`}>
            <p className="text-foreground leading-relaxed text-2xl">{point}</p>
          </div>
        ))}

        {/* Assessment */}
        <div className="animate-fade-up stagger-4 mt-4 p-6 rounded-lg bg-amber-500/10 border border-amber-500/30">
